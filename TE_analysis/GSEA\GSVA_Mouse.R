...
# [修改] 添加 eps = 0 以解决 "P-values likely overestimated" 警告
kk_gsea <- GSEA(gene_list, TERM2GENE = kegg_t2g, pvalueCutoff = 1, verbose = FALSE, seed = 123, eps = 0)

# [修改] OrgDb = org.Mm.eg.db
# [修改] 添加 eps = 0
go_gsea <- gseGO(geneList = gene_list, OrgDb = org.Mm.eg.db, ont = "BP", 
                 pvalueCutoff = 1, minGSSize = 5, verbose = FALSE, seed = 123, eps = 0)
...
if(nrow(plot_data) > 0) {
  ...
  p_dot <- ggplot(plot_data, aes(x = NES, y = Description_Label)) +
    # [修改] size -> linewidth (解决 ggplot2 警告)
    geom_segment(aes(x = 0, xend = NES, y = Description_Label, yend = Description_Label), color = "grey70", linewidth = 0.8) +
